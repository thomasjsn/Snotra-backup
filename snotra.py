...
if target_backend in ['ftp', 'rsync']:
    target_prefix = (
        '%(backend)s://%(uid)s@%(host)s:%(port)s/' % {'backend': target_backend, 'uid': host_user, 'host': target_host,
                                                      'port': target_port})
else:
...
def run_command(command, duplicity=False):
...
# Enumerate through all backup definitions.
for (i, item) in enumerate(Config.sections()):
    ...
    if enabled:
        ...
        run_command(cl_command, True)

        # Do duplicity backup.
        cp_command = (
            'duplicity %(full_if_older)s %(source)s %(prefix)s%(folder)s%(target)s %(exclude)s --log-file %(log)s' %
            {'full_if_older': full_if_older_str, 'source': source, 'prefix': target_prefix, 'folder': target_folder,
             'target': target, 'exclude': exclude_str, 'log': duplicity_log})
        run_command(cp_command, True)

        # If remove-if-older enabled, run command.
        if not remove_if_older == '':
            rm_command = (
                'duplicity remove-older-than %(remove_if_older)s --force %(prefix)s%(folder)s%(target)s --log-file %(log)s' %
                {'remove_if_older': remove_if_older, 'prefix': target_prefix, 'folder': target_folder, 'target': target,
                 'log': duplicity_log})
            run_command(rm_command, True)
